...
$label = array();
$arlen = 1;
array_push($sd,(float)${'SD_'.$courseid});
array_push($label,$year."".$sem);
	...
	while(1){
		...
		if($result){
			if($row=mysql_fetch_array($result)){
				//echo "reached 1";
				array_push($sd,(float)$row[2]);
				array_push($label,$curYear."Autumn");
				$arlen++;
			}
		}else{
			$tobreak=true;
		...
		if($result){
			if($row=mysql_fetch_array($result)){
				//echo "reached 2";
				array_push($sd,(float)$row[2]);
				array_push($label,$curYear."Spring");
				$arlen++;
			}
		}else{
			$tobreak=true;
