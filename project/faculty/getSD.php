                ...
                while($colIndex<$numCols){
                        if($rowIndex==1){
                                if($row[$colIndex]==null){
                                        ...
                                        array_push($averages,(float)pow(($row[$colIndex]-$averages[$colIndex-1]),2));
                                        //echo $row[$colIndex];
                                }
                        }else{
                                if($row[$colIndex]!=null)
                                        $sd[$colIndex-1] = $sd[$colIndex-1]+(float)pow(($row[$colIndex]-$averages[$colIndex-1]),2);
                        }
                $colIndex++;
                }
        $rowIndex++;
...
//echo $rowIndex;
while($index<=($numCols-2)){
$sd[$index] = sqrt($sd[$index]/($rowIndex-1));
echo "".$sd[$index].",";
$index++;
}
